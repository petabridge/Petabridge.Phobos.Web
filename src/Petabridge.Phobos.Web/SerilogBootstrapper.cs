...
namespace Petabridge.Phobos.Web
{
    ...
    public static class SeqBootstrapper
    {
        ...
        public static string LoadSeqUrl()
        {
            if (SeqEnabled)
            {
                return $"http://{System.Environment.GetEnvironmentVariable(SEQ_SERVICE_HOST)}:{System.Environment.GetEnvironmentVariable(SEQ_SERVICE_PORT)}";
            }
        ...
        /// <summary>
        /// Checks to see if the <see cref="SEQ_SERVICE_HOST"/> and <see cref="SEQ_SERVICE_PORT"/> environment variables are defined.
        /// </summary>
        public static bool SeqEnabled =>
            !string.IsNullOrEmpty(System.Environment.GetEnvironmentVariable(SEQ_SERVICE_HOST)) &&
            !string.IsNullOrEmpty(System.Environment.GetEnvironmentVariable(SEQ_SERVICE_PORT));

        public static string GetServiceName()
        {
            var podName = System.Environment.GetEnvironmentVariable(SerilogBootstrapper.PodNameProperty);
    ...
    {
        public static readonly Config SerilogConfig = @"akka.loggers =[""Akka.Logger.Serilog.SerilogLogger, Akka.Logger.Serilog""]";
